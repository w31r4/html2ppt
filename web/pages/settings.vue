<template>
  <div class="max-w-3xl mx-auto animate-fade-in px-4 md:px-0 py-6 md:py-0">
    <header class="mb-6 md:mb-8">
      <h1 class="text-2xl md:text-3xl font-semibold text-text-light dark:text-text-dark mb-2">Settings</h1>
      <p class="text-sm md:text-base text-gray-500 dark:text-gray-400">Manage your preferences and application configuration.</p>
    </header>

    <!-- Loading Skeleton -->
    <div v-if="isLoading" class="flex flex-col gap-4 md:gap-6">
      <div v-for="i in 3" :key="i" class="card p-4 md:p-5 animate-pulse">
    ...
    <div v-else class="flex flex-col gap-4 md:gap-6">
      <!-- Appearance Section -->
      <section class="card">
        <div class="p-4 md:p-5 border-b border-border-light dark:border-border-dark">
          <h3 class="text-base md:text-lg font-semibold text-text-light dark:text-text-dark">Appearance</h3>
        </div>
          ...
          <div class="mb-5">
            ...
            <div class="grid grid-cols-3 gap-2 md:gap-3">
              <label
                ...
                :key="option.value"
                class="border rounded-lg p-2 md:p-3 text-center cursor-pointer transition-all focus-within:ring-2 focus-within:ring-primary"
              ...
              >
                ...
                />
                <span class="material-symbols-rounded text-lg md:text-xl mb-1 block">{{ option.icon }}</span>
                <span class="text-xs md:text-sm">{{ option.label }}</span>
              </label>
            </div>
          </div>
        </div>
      ...
      <section class="card">
        <div class="p-4 md:p-5 border-b border-border-light dark:border-border-dark">
          <h3 class="text-base md:text-lg font-semibold text-text-light dark:text-text-dark">Generation Defaults</h3>
        </div>
        <div class="p-4 md:p-5 space-y-4 md:space-y-5">
          <div>
            ...
            <input
              ...
              min="1"
              max="50"
              class="w-full px-3 py-2.5 border border-border-light dark:border-border-dark rounded-lg bg-white dark:bg-card-dark text-text-light dark:text-text-dark focus:border-primary focus:ring-2 focus:ring-primary/30 outline-none transition-colors"
            />
          ...
          <div>
            ...
            <input
              ...
              class="w-full px-3 py-2.5 border border-border-light dark:border-border-dark rounded-lg bg-white dark:bg-card-dark text-text-light dark:text-text-dark placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/30 outline-none transition-colors"
            />
            <span class="text-xs text-gray-500 dark:text-gray-400 mt-1 block">
      ...
      <section class="card">
        <div class="p-4 md:p-5 border-b border-border-light dark:border-border-dark">
          <h3 class="text-base md:text-lg font-semibold text-text-light dark:text-text-dark">API Configuration</h3>
        </div>
        <div class="p-4 md:p-5 space-y-4 md:space-y-5">
          <div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 md:gap-4">
        <button
          class="btn btn-primary flex items-center justify-center gap-2 focus:ring-2 focus:ring-primary focus:ring-offset-2"
          @click="saveSettings"
          :disabled="saving"
        >
        ...
        <button
          class="btn btn-ghost flex items-center justify-center gap-2 focus:ring-2 focus:ring-gray-400 focus:ring-offset-2"
          @click="resetDefaults"
        >
...
onMounted(async () => {
  ...
  await settingsStore.fetchServerSettings();
...
const saveSettings = async () => {
  ...
  try {
    ...
    saveMessage.value = `Failed to save settings: ${errorMessage}`;
  } finally {
    saving.value = false;
    setTimeout(() => {
      saveMessage.value = '';
